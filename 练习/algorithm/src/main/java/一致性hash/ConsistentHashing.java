...
/**
 * 参考资料: https://github.com/whatColorSSS/learnning/tree/master/learnning/src/main/java/com/dutycode/learning/consistenthash
 */
public class ConsistentHashing {
    ...
    // 每个真实节点对应的虚拟节点个数
    private static int SINGLE_VISUAL_NUM =1000;
...
//        initRadomData2VisualNodeMy();
    ...
    private static void initRadomData2VisualNode() {
        ...
        for (int i = 0; i < 1000000; i++) {
            String dataKey = DigestUtils.md5DigestAsHex(String.valueOf(random.nextInt()).getBytes());
        ...
        for (String s : dataCache.keySet()) {
            CacheNode item = dataCache.get(s);
            l.add(item.getNodeName());
        }
        List<String> disnit = l.stream().distinct().collect(Collectors.toList());
        Map<String, List<String>> sssss = l.stream().collect(Collectors.groupingBy(Function.identity()));
        int max = 1;
        for (List<String> value : vNodeCacheDataMap.values()) {
            if (value.size()>max) {
                max = value.size();
            }
        }
    ...
    private static void initRadomData2VisualNodeMy() {
        try {
            List<String> list = Files.readAllLines(Paths.get("D:\\datas\\secureCRTdown\\360_compare_files.txt"));
//            List<String> list = Files.readAllLines(Paths.get("D:\\datas\\secureCRTdown\\46805.txt"));
            List<String> aa = list.stream().filter(e-> e.length()>1).map(e -> e.split(",")[2]).collect(Collectors.toList());
            for (String s : aa) {
                String dataKey = DigestUtils.md5DigestAsHex(s.getBytes());
                VisualCacheNode nearestCacheNode = getVisualCacheNode(dataKey);
                // 将key值和真实缓存节点的关联关系存储起来
                dataCache.put(dataKey, nearestCacheNode.getRealCacheNode());
    ...
    private static void addCacheNode() {
        ...
        for (VisualCacheNode vnode : addVisualNodeList) {
            ...
            String md5VisualNodeKey = DigestUtils.md5DigestAsHex(nodeName.getBytes());// 使用common-codec工具包
    ...
    private static void removeCacheNode(String nodeIp) {
        ...
        for (VisualCacheNode c : vNodesList) {
            ...
            String md5VisualNodeKey = DigestUtils.md5DigestAsHex(vNodeName.getBytes());// 使用common-codec工具包
            int hashNodeName = DJBHash(md5VisualNodeKey);
    ...
    private static List<VisualCacheNode> genVisualCacheNode(CacheNode cacheNode) {
        ...
        for (int i = 0; i < SINGLE_VISUAL_NUM; i++) {
            ...
            // 对虚拟节点的key值做下md5
            String md5VisualNodeKey = DigestUtils.md5DigestAsHex(visualNodeKey.getBytes());// 使用common-codec工具包
    ...
    private static void addVisualCacheNodeIntoCircle(List<VisualCacheNode> vNodesList) {
        for (VisualCacheNode vNode : vNodesList) {
            // 对虚拟节点的key值做下md5
            String md5VisualNodeKey = DigestUtils.md5DigestAsHex(vNode.getNodeName().getBytes());// 使用common-codec工具包
            int hashKey = DJBHash(md5VisualNodeKey);
